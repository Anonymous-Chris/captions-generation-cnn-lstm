...
POLOS was important because it gave me a second learned semantic perspective in addition to CLIPScore.

Approximate Pearson correlations with POLOS were:
...
Approximate Spearman correlation between CLIPScore and POLOS was:
...
> POLOS aligned much more strongly with CLIPScore than with most traditional lexical metrics. This suggests that POLOS and CLIPScore capture similar semantic properties, while still not being identical.

Interview version:

> One interesting result was that POLOS had about 0.80 Pearson correlation with CLIPScore, which was much higher than the correlation between CLIPScore and BLEU-style metrics. That gave me stronger evidence that semantic metrics capture a different dimension of caption quality than exact lexical overlap.
...
> The generated caption used wording that differed from the reference captions, so BLEU-4 was low, but both CLIPScore and POLOS judged it much more favorably.

Research significance:

> This is stronger evidence than relying on CLIPScore alone because two different learned semantic metrics agreed that the caption was relatively good despite weak lexical overlap.
...
> Lexical similarity can sometimes produce a relatively strong score even when semantic alignment is weak.

Research significance:

> Because both CLIPScore and POLOS were low, this case suggests that the issue was not just one unusual metric score; the semantic evaluators were jointly signaling a mismatch.
...
# Role of POLOS in the Perturbation Study

POLOS was also evaluated on the same controlled perturbations as the traditional metrics and CLIPScore.

The perturbation study therefore compares:
...
> Including both CLIPScore and POLOS made the robustness experiment stronger because I was not relying on only one semantic metric. I could compare whether two learned semantic evaluators reacted similarly to controlled meaning changes.
...
> Traditional lexical metrics and learned semantic metrics capture different but complementary aspects of caption quality.

The traditional metrics showed only moderate correlations with CLIPScore, while POLOS showed a much stronger relationship with CLIPScore.
...
POLOS   vs CLIPScore ≈ 0.803
```

This suggests that the two learned semantic metrics capture more similar information, while lexical overlap emphasizes a different aspect of caption quality.
...
> The metrics are positively correlated, but they are not interchangeable. Traditional metrics showed only moderate agreement with CLIPScore, while POLOS showed much stronger agreement with CLIPScore. That suggests semantic metrics capture related information that is different from exact lexical overlap, so a multi-metric evaluation framework is more informative than relying on a single score.
...
> My research focused on the reliability of automatic evaluation metrics for image captioning. I first implemented a CNN-LSTM captioning baseline on Flickr8k using pretrained VGG16 features and an LSTM decoder. I then evaluated the generated captions using BLEU, METEOR, ROUGE-L, CIDEr, CLIPScore, and POLOS. The main question was whether traditional lexical metrics and learned semantic metrics measure the same aspects of caption quality. I performed Pearson and Spearman correlation analysis, rank-disagreement analysis, and qualitative case studies. One important result was that POLOS correlated strongly with CLIPScore, at about 0.80 Pearson correlation, while traditional metrics showed only moderate correlations with CLIPScore. I also designed controlled perturbation experiments involving object changes, attribute changes, action changes, negation, and hallucination. The main finding was that lexical and semantic metrics are related but not interchangeable, so using multiple complementary metrics gives a more reliable evaluation.
...
>
> I first performed Pearson and Spearman correlation analysis. The traditional metrics had positive but moderate correlations with CLIPScore. For example, BLEU-4 and CLIPScore had a Pearson correlation of about 0.39. In contrast, POLOS and CLIPScore had a much stronger Pearson correlation of about 0.80 and a Spearman correlation of about 0.82. That suggested that the learned semantic metrics were capturing more similar properties, while exact lexical overlap was emphasizing something different.
>
...
> I studied the reliability of automatic evaluation metrics for image captioning. I used a CNN-LSTM captioning system as a baseline, evaluated its captions with lexical metrics and two learned semantic metrics, CLIPScore and POLOS, analyzed correlations and ranking disagreements, and then designed controlled perturbation experiments to test sensitivity to semantic errors. POLOS showed strong agreement with CLIPScore while both differed more from traditional lexical metrics, which supported the conclusion that semantic and lexical evaluation capture different aspects of caption quality.
